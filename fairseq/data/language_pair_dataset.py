...
class LanguagePairDataset(FairseqDataset):
    ...
    def num_tokens(self, index):
        ...
        max_size_1 = self.src_sizes[index] + self.tgt_sizes[index] if self.tgt_sizes is not None else 0
        max_size_2 = self.src2_sizes[index] if self.src2_sizes is not None else 0 + \
                        self.tgt2_sizes[index] if self.tgt2_sizes is not None else 0
        return max(max_size_1, max_size_2)
